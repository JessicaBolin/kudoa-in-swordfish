# Test two methods of detecting the EAC via PCA 
# Author:       Jessica Bolin (jessica.anne.bolin@gmail.com)
# Last updated: 2024-02-01

### Methods to detect EAC
# Based on a priori chosen combinations of variables to represent the 
# surface signature of the EAC, as per Bolin et al 2020 RSEC
# (i) SST, SSH, V, Speed
# (ii) SST, V, speed
...
dir = "PATH" 
# Path where daily regridded ACCESS files are stored (i.e., one file per day)
...
for (k in 1:length(datee)) { 
  ...
  sst <- raster(paste0(dir, "sst/", sst_f)) %>% crop(area)
  v <- raster(paste0(dir, "v/", v_f)) %>% crop(area)
  u <- raster(paste0(dir, "u/", u_f)) %>% crop(area)
  speed <- sqrt(u^2 + v^2) %>% crop(area)
  ssh <- raster(paste0(dir, "ssh_corrected/", ssh_f)) %>% crop(area)
  ...
  dat <- data.frame(sst = sst.v, v = v.v, speed = speed.v, ssh = ssh.v, 
                    x = xy$x, y = xy$y)
  dat <- na.omit(dat) 
  # Scale values to mean and unit variance, no center as value as 0 needs to be meaningful
...
inversed1 #[1] 23 days with inverse loadings
100 - (inversed1/nrow(df) * 100) #[1] 91.75627 % hit rate

# PC1 variance stats
mean(all.stuff$variance) # [1] 0.7002509
median(all.stuff$variance) # [1] 0.7
sd(all.stuff$variance) #[1] 0.02312637
...
inversed2 #[1] 21 days with inversed loadings (failures)
100 - (inversed2/nrow(df) * 100) # [1] 92.47312 % hit rate 

# PC1 variance stats
mean(all.stuff$variance) #[1] 0.6484588
median(all.stuff$variance) #[1] 0.64
sd(all.stuff$variance) #[1] 0.0405465
all.stuff$variance %>% range #[1] 0.58 0.75
...
cat("mean of method 1:", m1_meanvar) #mean of method 1: 0.7002509
cat("sd of method 1:", m1_sdvar) #sd of method 1: 0.02312637
...
cat("mean of method 2:", m2_meanvar) #mean of method 2: 0.6484588
cat("sd of method 2:", m2_sdvar) #sd of method 2: 0.0405465

# Chose method 1. Has slightly worse hit rate, but higher mean variance 
# explained by PC1. 
